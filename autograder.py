    ...
    for r in score_by_results:
        ...
        while flag:
            ...
            if score_by_results[r][answer_match] < 0.45:
                ...
                flag = False
            elif (max_result[0] == r) or (
                score_by_results[r][answer_match] > score_by_answers[answer_match][max_result[0]]
            ):
...
def score_structured(year, answers, info_type):
    ...
    for a in answers["award_data"]:
        if info_type == "winner":
            ...
            temp_spelling, translation = calc_translation(results[a], answers["award_data"][a][info_type])
            c_score += calc_score(
                [translation[res] if res in translation else res for res in results[a]],
